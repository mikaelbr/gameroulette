	...
	* InGame is made.  Default is increment level and stage. */
	...
	public void startGame(int level_selected) {
		...
		if (startgame_ticks > 0) {
			...
			new JGTimer(startgame_ticks,true,"StartLevel", this) {
	...
	public final void lifeLost() {
		...
		if (lifelost_ticks > 0) {
			...
			new JGTimer(lifelost_ticks,true,"LifeLost", this) { public void alarm() {
				endLifeLost();
			} };
		} else {
			endLifeLost();
	...
	private void endLifeLost() {
		...
		if (lives <= 0) {
			...
			if (startgame_ticks > 0) {
				...
				if (startgame_ingame) addGameState("InGame");
				new JGTimer(startgame_ticks,true,"StartGame", this) {
					public void alarm() { setGameState("InGame"); } };
			} else {
				// force call to startInGame()
				clearGameState();
	...
	public final void levelDone() {
		...
		if (leveldone_ticks > 0) {
			...
			new JGTimer(leveldone_ticks,true,"LevelDone", this) {public void alarm() {
	...
	*/
	public final void gameOver() {
		...
		if (gameover_ticks > 0) {
			...
			new JGTimer(gameover_ticks,true,"GameOver", this) {
